...
#ifdef SIMPLE_PY
    if (m_IMM.Open("./gbkpy.mb") != TRUE)
    {
        qDebug("m_IMM.Open failed\n");
    }
    else
    {
        qDebug("m_IMM.Open success\n");
    }
#endif
}
...
void frmInput::selectChinese()
{
    ...
    //qDebug("py: %s\n", currentPY.toUtf8().constData());
    currentPY_count = m_IMM.Filter(currentPY.toUtf8().constData());

    int i = 0;
    unsigned short code = 0;
    char buf[8];

    for (i=0; i<currentPY_count; i++)
    {
    ...
    QString str = allPY[0];
    char buf[10];
    strcpy(buf, str.toUtf8().constData());
    printf("%s: 0x%2x 0x%2x 0x%2x\n", str.toUtf8().constData(), buf[0], buf[1], buf[2]);
    //
    #endif
    showChinese();
}
    ...
    if (objectName == "btnType") {
        ...
        if (currentType != "chinese") {
            ...
            if (btn->property("btnOther").toBool()) {
                ...
                ui->labPY->setText(ui->labPY->text() + value);//拼音
                selectChinese();
            }
        }
    }
...
{
    QFont btnFont("wenquanyi", btnFontSize);
    QFont labFont("wenquanyi", labFontSize);
