...
int SetSockLinkOpt(SOCKHANDLE hSock)
{
	int ret;
		...
		if(hServerSock != INVALID_SOCKET && FD_ISSET(hServerSock,&set))
		{
			...
			if(hTmp != INVALID_SOCKET)
			{
				...
				else if (0x2 == Conntype)
				{
					...
					u8 buf[256] = {0};
					int ret=looprecv(hTmp, (char *)buf,sizeof(ifly_TCP_Stream_Req));
...
void *CPAckSearchProc(void *pParam)
{
	...
	else if (type == CTRL_DEVICESEARCH_ACKCLIENT)
	{
		...
		//DBG_PRINT("CTRL_DEVICESEARCH_ACKCLIENT port: %d\n", g_wCliAckPort);
...
#else
		hCPSearchTask = IFly_CreateThread(CPAckSearchProc,NULL,PRI_CPCHECK,STKSIZE_CPCHECK,(uint)&acktype,0,NULL);
#endif
...
int SearchDevice()
{
	...
	{
		SockClose(serSocket);
		ERR_PRINT("socket SO_REUSEADDR failed\n");
		return -1;
	}
	...
	{
		SockClose(serSocket);		
		ERR_PRINT("socket IP_ADD_MEMBERSHIP failed\n");
		return -1;
	}
...

	{
		...
		{
			ERR_PRINT("socket send multicast package failed\n");	
			return -1;
		}
#ifndef WIN32
		usleep(1000*1000);
